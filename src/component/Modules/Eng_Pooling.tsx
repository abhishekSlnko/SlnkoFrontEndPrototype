...
import { useGetPoolingsQuery } from "../../redux/Eng/poolingsSlice";

const PoolingTab = forwardRef((props, ref) => {
  ...
  const { data: getLead = [], isLoading, error } = useGetPoolingsQuery();
  ...
  useImperativeHandle(ref, () => ({
    exportToCSV() {
      ...
      const headers = ["Category", "Item Name", "Rating", "Status"];
      ...
      link.download =
        selected.length > 0
          ? "Selected_Pooling Station.csv"
          : "All_Pooling Stations.csv";
      link.click();
    },
  }));
        ...
        >
          <Grid container spacing={2}>
            ...
            <Grid xs={12} sm={6}>
              ...
              <Input
                name="size"
                value={selectedLead?.technicalSpecification ?? ""}
                readOnly
                fullWidth
              />
            </Grid>
